...
public class RulersIText extends ElementoIText implements Rulers {
  ...
  public RulersIText(XReport stampa, RulersElement lineElem, Elemento padre) throws GenerateException {
    setParent(padre);
    c_stampa = stampa;
  ...
  public void draw(PdfWriter writer) throws GenerateException {
    ...
    if (col == null) {
      ...
      if (baseColor == null) {
        throw new GenerateException(c_rulersElem, "Colore non definito: " + col.getName());
      }
    }
    ...
    try {
      LineaIText.applyLineStyle(cb, c_rulersElem.getThickness().getValue(),
          LineaIText.Style.valueOf(c_rulersElem.getStyle().toUpperCase()), baseColor);
      ...
      BaseFont font = BaseFont.createFont(BaseFont.COURIER, BaseFont.WINANSI, true);
      ...
      while (hPos < heiDoc) {
        if (hPos > 0) {
          ...
          if (showText) {
            ...
            cb.showTextAligned(Element.ALIGN_LEFT, nf.format(hPos), 10, hPos+1, 0);
            cb.endText();          	
          }
        }
        hPos += dist;
      ...
      while (vPos < widDoc) {
        if (vPos > 0) {
          ...
          if (showText) {
            cb.beginText();
            cb.setFontAndSize(font, 6);
            cb.showTextAligned(Element.ALIGN_CENTER, nf.format(vPos), vPos, heiDoc - 10, 0);
            cb.endText();           
          }
        }
        vPos += dist;
      ...
      e.printStackTrace();
      throw new GenerateException("Errore grave in disegno rulers: " + e.toString());
    }
    finally {
      cb.restoreState();
    }
