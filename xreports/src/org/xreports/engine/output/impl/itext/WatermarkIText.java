...
public class WatermarkIText extends ElementoIText implements Watermark {
	...
	public WatermarkIText(XReport stampa, WatermarkElement wElem, Elemento padre)
    ...
    try {      
  		...
  		String text = c_watermarkElem.getText(c_stampa);
      if (text != null)
  ...
  private void printText(PdfWriter writer, Document document, String text) throws GenerateException {
    ...
    if (c_watermarkElem.isFitText()) {
      //devo calcolare la dimensione della font sufficiente per riempire in larghezza:
      ...
      for (int i=0; i < texts.length; i++) {
        //se testo su più linee, calcolo font per ogni linea, alla fine prendo la più piccola
        float textSize = getFontSizeToFit(document, texts[i], textDiagonal, f.getBaseFont(), 16, 250);
        if (textSize < sizeOK)
          sizeOK = textSize;
      }            
    ...
    float refPointY = calcRefPointY(document, textHeight, alignment==Element.ALIGN_CENTER);
...
//    leading += (maxAscent + maxDescent) * Math.sin(angleRadians);  //interlinea aumenta in proporzione all'angolo di rotazione
    float stepHeight = leading * (float)Math.cos(angleRadians);
    float stepWidth = leading * (float)Math.sin(angleRadians);
    if (texts.length > 1 && !(!c_watermarkElem.isFitText() && c_watermarkElem.getHalign()==HAlign.LEFT)) {
      float linesHeight = (leading * texts.length) * (float)Math.cos(angleRadians);
      float linesWidth = (leading * texts.length) * (float)Math.sin(angleRadians);
    ...
    if (textColor != null) {
      ...
      fOK.setColor(new BaseColor(textColor.getRed(), textColor.getGreen(), textColor.getBlue(), textColor.getAlpha()));
    }
    ...
    float rotation = writer.getPageNumber() % 2 == 1 ? angleGrades: -angleGrades;
    for (int i=0; i < texts.length; i++) {
...
//    ColumnText.showTextAligned(cb, Element.ALIGN_CENTER, new Phrase(pp, fText),
//        refPointX, refPointY - 3, 0);
  }
  
  /**
   * In base alla dimensione del documento, margini e VAlign, calcola l'atezza del punto di riferimento.
   * @param document docuemnto pdf
   * @param blockSize altezza del blocco di testo o dell'immagine
   * @param centered se true, indica che l'ordinata di riferimento è centrata rispetto a blockSize 
   * @return ordinata del punto di riferimento
   */
  private float calcRefPointY(Document document, float blockSize, boolean centered) {
    float refPointY;
...
//        }
  ...
  /**
   * Calcola la dimensione della font perchè il testo passato abbia una larghezza il più vicino 
   * possibile 'sizeToFit' ma non superiore.
   * 
   * @param text testo di cui calcolare la larghezza
   * @param sizeToFit larghezza a cui deve arrivare il testo
   * @param bf basefont da usare
   * @param minSize dimensione minima della font
   * @param maxSize dimensione massima della font
   * @return dimensione che deve avere la BaseFont perchè il testo passato sia largo maxSize;
   *   NB: la dimensione ritornata è sempre compresa nel range minSize-maxSize.
   */
  private float getFontSizeToFit(Document document, String text, double sizeToFit, BaseFont bf, int minSize, int maxSize) {
    float  angleGrades = c_watermarkElem.getRotation();
    double angleRadians = Math.abs(gradiToRadianti(angleGrades));    
    ...
    while (currentSize <= maxSize) {
      ...
      float heightOfText = testSize + (float)size * (float)Math.sin(angleRadians);
   ...
   * 
   * @throws IOException in caso ci sia un errore in letture del file dell'immagine
   * @throws DocumentException nel caso ci sia un errore a basso livello in aggiunta immagine al documento pdf
   * @throws GenerateException  nel caso il file dell'immagine specificato nell'attributo 'image' non sia trovato
   */
  private void printImage(PdfWriter writer, Document document) throws IOException, DocumentException, GenerateException {
    String absPathImage = c_stampa.findResource(c_watermarkElem.getImage());
    if (absPathImage == null) {
      throw new GenerateException(c_watermarkElem, "Non riesco a trovare il file di immagine " + c_watermarkElem.getImage());
    }
...
//        (document.getPageSize().getHeight() - img.getScaledHeight()) / 2);   
  ...
  /**
   * Ridimensiona immagine se almeno uno fra attributi 'width' e 'height'
   * è specificato
   * @param img immagine da scalare
   */
